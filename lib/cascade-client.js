...
// Per-request TLS agent for self-signed LS certificates (SEC-1 fix)
let _tlsAgent = null;
function getTlsAgent() {
    if (!_tlsAgent) _tlsAgent = new https.Agent({ rejectUnauthorized: false });
    return _tlsAgent;
}
...
async function callApi(method, body = {}) {
    ...
    // Per-request TLS for self-signed certs (replaces global NODE_TLS_REJECT_UNAUTHORIZED)
    if (c.useTls) opts.dispatcher = getTlsAgent();
    const res = await fetch(makeUrl(method), opts);
...
// Get all cascade statuses (cached for 1s — PERF-1 fix)
...
export async function getAllStatuses() {
    const now = Date.now();
    if (_statusCache && now - _statusCacheTime < STATUS_CACHE_TTL_MS) {
        return _statusCache;
    }
    _statusCache = await callApi('GetAllCascadeTrajectories', {});
