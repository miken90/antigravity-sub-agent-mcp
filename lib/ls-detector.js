...
// @visibleForTesting
export function parseProcessOutput(stdout) {
    const instances = [];
    if (platform === 'win32') {
...
export function detectPorts(pid) {
    const safePid = parseInt(pid, 10); // SEC-3: sanitize PID
    if (isNaN(safePid)) return Promise.resolve([]);
    return new Promise((resolve) => {
        let cmd;
        if (platform === 'win32') {
            // PERF-2 + SEC-4: filter at OS level instead of parsing full table
            cmd = `netstat -ano | findstr /C:"${safePid}"`;
        } else {
...
// @visibleForTesting
export function pathToWorkspaceId(absPath) {
...
// @visibleForTesting
export function workspaceIdMatchesPath(workspaceId, targetPath) {
...
async function detectFromParentProcess() {
    const ppid = parseInt(process.ppid, 10); // SEC-3: sanitize PID
    if (!ppid || isNaN(ppid)) return null;
