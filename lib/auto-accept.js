...
// SEC-7: Block obviously destructive commands from auto-accept
// @visibleForTesting
export const BLOCKED_COMMANDS = [
    /^rm\s+-rf\s+\/\s*$/,           // rm -rf /
    /^rm\s+-rf\s+\/[^\s]+/,         // rm -rf /<dir> (root-level recursive delete)
    /^(format|shutdown|reboot|halt)\b/i,
...
export function isBlockedCommand(cmd) {
    return BLOCKED_COMMANDS.some(re => re.test(cmd.trim()));
}
