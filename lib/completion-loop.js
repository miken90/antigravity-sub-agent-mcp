...
// PERF-3: Adaptive polling — faster early, slower later to reduce API load
const POLL_INTERVAL_MIN_MS = 1000;         // First 5s: check every 1s
const POLL_INTERVAL_MID_MS = 2000;         // 5-30s: check every 2s
const POLL_INTERVAL_MAX_MS = 5000;         // 30s+: check every 5s
const MAX_POLL_DURATION_MS = 600000;       // 10 min timeout
const STALL_THRESHOLD_THINKING = 120;      // ~60s patience while RUNNING with steps
const STALL_THRESHOLD_NO_PROGRESS = 60;    // ~30s patience if zero steps ever produced
const MAX_AUTO_REPLIES = 3;                // Max auto-replies to questions

// @visibleForTesting
export function getPollInterval(elapsedMs) {
    if (elapsedMs < 5000) return POLL_INTERVAL_MIN_MS;
    if (elapsedMs < 30000) return POLL_INTERVAL_MID_MS;
...
export async function waitForCompletion(cascadeId, options = {}) {
    ...
    while (Date.now() - startTime < timeoutMs) {
        await sleep(getPollInterval(Date.now() - startTime));
...
// @visibleForTesting
export function isAskingQuestion(step) {
